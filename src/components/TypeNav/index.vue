<template>
  ...
  <div class="type-nav">
    <div class="container">
      <!-- 事件委派 | 事件委托，把本应该放在h3的事件 ，提升到父容器中，让父容器中的所以元素都能触发该事件（@mouseleave） -->
...
export default {
  ...
  mounted() {
    ...
    //派发categoryList方法
    // this.$store.dispatch("categoryList"); // 放到App.vue中，让其只发一次请求，减轻服务器的压力
    //当组件挂载完毕，如果不是home路由组件
    //就让show改为false
  ...
  methods: {
    ...
    //鼠标进入，修改响应式数据 currentIndex 的属性值
    // changeIndex(index) {
